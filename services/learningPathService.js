 ...
 * @param {Array<object>} clusters - The array of clusters and their steps generated by the AI.
 * @returns {object} The newly created learning path object.
 */
export function addPath(goal, clusters) {
    const flatPath = clusters.flatMap(cluster => cluster.steps);
    const newPath = {
        id: `path_${Date.now()}`,
        goal,
        clusters,
        path: flatPath, // The flattened path for linear progress tracking
        createdAt: new Date().toISOString(),
        currentStep: 0,
        stepScores: {},
        clusterReviewScores: {}, // New property for review scores
    };
...
/**
 * Records the user's score for a "Cluster Review Quiz".
 * @param {string} pathId - The ID of the learning path.
 * @param {number} clusterIndex - The index of the cluster being reviewed.
 * @param {number} score - The number of correct answers.
 * @param {number} totalQuestions - The total number of questions in the quiz.
 */
export function recordClusterReviewScore(pathId, clusterIndex, score, totalQuestions) {
    const path = getPathById(pathId);
    if (path) {
        if (!path.clusterReviewScores) {
            path.clusterReviewScores = {};
        }
        path.clusterReviewScores[clusterIndex] = { score, totalQuestions };
        savePaths();
    }
}
